	var callbackReq = function(url, success, error) {
	...
	var path = 'firstAsync'; //errorResponse firstAsync
	...
	/////////////// Callback Async ///////////////
	/*try {
		callbackReq(url + path, function(result) {
			try {
				callbackReq(url + result, function(result) {
	...
	function *mainPromise() {
		...
		var mainResult2 = yield promiseReq(url + mainResult1);
...
var runGenerator = function(gen) {
	...
	function requestCallback(url) {
		callbackReq(url, function(response) {
	...
	/*try {
		callbackReq(url + path, function(data) {
			callbackReq(url + data, function(data2) {
	...
	try {
		var value = promiseReq(url + path);
		value
		.then(function(data) {
			try {	
				return promiseReq(url + data);
			}
			catch (err) {
